#include "ps/equity_calc.h"

#include "ps/cards.h"


#include "ps/detail/visit_combinations.h"

#include <boost/format.hpp>

namespace ps{
                
bool equity_calc::run( bnu::matrix<size_t>& result,
                       std::vector<holdem_id> const& players,
                       std::vector<card_id> const& board,
                       std::vector<card_id> const& dead)noexcept
{
        switch( players.size()){
        case 2: return run_p<RunType_Normal, 2>( result, players, board, dead ); break;
        case 3: return run_p<RunType_Normal, 3>( result, players, board, dead ); break;
        case 4: return run_p<RunType_Normal, 4>( result, players, board, dead ); break;
        case 5: return run_p<RunType_Normal, 5>( result, players, board, dead ); break;
        case 6: return run_p<RunType_Normal, 6>( result, players, board, dead ); break;
        case 7: return run_p<RunType_Normal, 7>( result, players, board, dead ); break;
        case 8: return run_p<RunType_Normal, 8>( result, players, board, dead ); break;
        case 9: return run_p<RunType_Normal, 9>( result, players, board, dead ); break;
        default:
                return false;
        }
}
bool equity_calc::run_ordered( bnu::matrix<size_t>& result,
                               std::vector<holdem_id> const& players,
                               std::vector<card_id> const& board,
                               std::vector<card_id> const& dead)noexcept
{
        switch( players.size()){
        case 2: return run_p<RunType_Ordered, 2>( result, players, board, dead ); break;
        case 3: return run_p<RunType_Ordered, 3>( result, players, board, dead ); break;
        case 4: return run_p<RunType_Ordered, 4>( result, players, board, dead ); break;
        case 5: return run_p<RunType_Ordered, 5>( result, players, board, dead ); break;
        case 6: return run_p<RunType_Ordered, 6>( result, players, board, dead ); break;
        case 7: return run_p<RunType_Ordered, 7>( result, players, board, dead ); break;
        case 8: return run_p<RunType_Ordered, 8>( result, players, board, dead ); break;
        case 9: return run_p<RunType_Ordered, 9>( result, players, board, dead ); break;
        default:
                return false;
        }
}
                
template<equity_calc::RunType type, size_t Num_Players>
bool equity_calc::run_p(bnu::matrix<size_t>& result,
                        std::vector<holdem_id> const& players,
                        std::vector<card_id> const& board,
                        std::vector<card_id> const& dead)noexcept
{
        auto dealt{ board.size() + dead.size() };
        auto to_deal{ 5- dealt  };
        switch(to_deal){
        case 1: return run_pd<type, Num_Players, 1>(result, players, board, dead);
        case 2: return run_pd<type, Num_Players, 2>(result, players, board, dead);
        case 3: return run_pd<type, Num_Players, 3>(result, players, board, dead);
        case 4: return run_pd<type, Num_Players, 4>(result, players, board, dead);
        case 5: return run_pd<type, Num_Players, 5>(result, players, board, dead);
        default:
                return false;
        }
}
template<equity_calc::RunType type, size_t Num_Players, size_t Num_Deal>
bool equity_calc::run_pd( bnu::matrix<size_t>& result,
                          std::vector<holdem_id> const& players,
                          std::vector<card_id> const& board,
                          std::vector<card_id> const& dead)noexcept{

        std::vector<id_type> known;

        // cache the cards
        std::array<id_type, Num_Players> x;
        std::array<id_type, Num_Players> y;

        size_t sigma{0};
        /* create matrix of draws */
        std::array<
                std::array<size_t,9>,
                9
        > win_matrix;

        std::array<double,Num_Players> equity;

        for( size_t i{0}; i!= Num_Players;++i){
                auto const& p{ players[i]};
                x[i]      = holdem_hand_decl::get(players[i]).first().id();
                y[i]      = holdem_hand_decl::get(players[i]).second().id();
                for( size_t j=0;j!=win_matrix.size();++j)
                        win_matrix[i][j]  = 0;
                equity[i] = 0.0;
        }

        boost::copy( x, std::back_inserter(known));
        boost::copy( y, std::back_inserter(known));
        boost::copy( board, std::back_inserter(known));
        boost::copy( dead , std::back_inserter(known));
        boost::sort(known);
        auto filter = [&](long c){ return ! boost::binary_search(known, c); };

        auto do_eval = [&](id_type a, id_type b, id_type c, id_type d, id_type e){
                std::array<std::pair<std::uint32_t, size_t >, Num_Players > ranked;
                ++sigma;
                for(size_t i{0};i!=Num_Players;++i){
                        ranked[i] = std::make_pair(eval_(x[i],y[i],a,b,c,d,e), i);
                }
                boost::sort( ranked, [](auto const& l, auto const& r){ return l.first < r.first; });
                auto winning_rank{ ranked.front().first };
                auto iter{ boost::find_if( ranked, [&](auto const& _){ return _.first != winning_rank; } ) }; 
                auto num_winners{ std::distance( ranked.begin(), iter) };

                for( auto j{ ranked.begin() }; j!=iter;++j){
                        ++win_matrix[j->second][num_winners - 1];
                        equity[j->second] += 1.0 / num_winners;
                }
        };

        id_type start{ 51 };
        if( type == RunType_Ordered  && board.size() ){
                start = *boost::min_element( board ) -1;
        }

        switch(board.size()){
        case 0:
                detail::visit_combinations<5>( [&](id_type a, id_type b, id_type c, id_type d, id_type e){
                        do_eval(a,b,c,d,e);
                }, filter, start);
                break;
        case 1:
                detail::visit_combinations<4>( [&](id_type a, id_type b, id_type c, id_type d){
                        do_eval(board[0], a,b,c,d);
                }, filter, start);
                break;
        case 2:
                detail::visit_combinations<3>( [&](id_type a, id_type b, id_type c){
                        do_eval(board[0], board[1], a,b,c);
                }, filter, start);
                break;
        case 3:
                detail::visit_combinations<2>( [&](id_type a, id_type b){
                        do_eval(board[0], board[1], board[2], a,b);
                }, filter, start);
                break;
        case 4:
                detail::visit_combinations<1>( [&](id_type a){
                        do_eval(board[0], board[1], board[2], board[3], a);
                }, filter, start);
                break;
        case 5:
                do_eval( board[0], board[1], board[2], board[3], board[4]);
                break;
        default:
                BOOST_THROW_EXCEPTION(std::domain_error("bad number of board cards"));
        }

        if( result.size1() != Num_Players ||
            result.size2() != computation_size ){
                result = bnu::matrix<size_t>{Num_Players, computation_size, 0};
        }

        for(size_t i{0};i!=Num_Players;++i){
                for(size_t j=0;j!= win_matrix[i].size();++j){
                        result(i, j)   = win_matrix[i][j];
                }
                result(i,9) = sigma;
                result(i,10) = equity[i] * computation_equity_fixed_prec;
        }
        return true;
}

} // ps
